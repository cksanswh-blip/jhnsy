...
export function validateRegistry(registry){
 ...
 for(const x of registry.invitations){
  ...
  if(Object.keys(x).some(k=>!['slug','status','expiresAt','title','description','target','image'].includes(k)))throw Error('Private/unknown fields cannot enter Git');
  if(x.status!=='published')continue;
  for(const field of ['target','image']){const u=new URL(x[field]);if(u.origin!=='https://egofathomin.com'||u.username||u.password||u.search||u.hash||!u.pathname.startsWith('/wedding/'))throw Error('Invalid public destination');}
...
export function renderShare(x,now=new Date()){
 ...
 return `<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,noarchive"><meta name="referrer" content="no-referrer"><title>${escape(x.title)}</title><meta property="og:type" content="website"><meta property="og:title" content="${escape(x.title)}"><meta property="og:description" content="${escape(x.description)}"><meta property="og:url" content="${share}"><meta property="og:image" content="${escape(x.image)}"><meta property="og:image:width" content="1200"><meta property="og:image:height" content="630"><meta name="twitter:card" content="summary_large_image"></head><body><main><h1>${escape(x.title)}</h1><p>${escape(x.description)}</p><a href="${escape(x.target)}">청첩장 열기</a></main><script>if(Date.now()>=Date.parse(${safeJson(x.expiresAt)})){document.title='초대가 마무리되었습니다';document.querySelector('main').textContent='청첩장 공개 기간이 종료되었습니다. 함께해 주셔서 감사합니다.';document.querySelectorAll('meta[property^="og:"]').forEach(x=>x.remove());}else{location.replace(${safeJson(x.target)});}</script></body></html>`;
}
